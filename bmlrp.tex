...
\begin{algorithm}
    \begin{algorithmic}[1]
        ...
        \Function{NH}{$a$}   \Comment{returns neighborhood of node $a$}
...
\begin{algorithm}
    \begin{algorithmic}[1]
        ...
        \Procedure{ConnectNeighborhoods}{$a, b$}
            \If {${\rm Used}[b]$} 
...
%\begin{equation*}
%\begin{split}
%      & \sum_{i = 0}^{k} \frac{q^i}{\sqrt{2}q - 1} < \frac{q^k}{\sqrt{2}q - 1} \frac{q}{q - 1} <        \\
%    < & \frac{q}{ (\sqrt{2}q - 1)(q - 1) } q^{log_2N} = \frac{q}{ (\sqrt{2}q - 1)(q - 1) } N^{log_2q}   \\
%\end{split}
%\end{equation*}
%
