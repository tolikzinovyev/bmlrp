...
\begin{defn}
    We say that node $a$ is connected to node $b$ on level $i$ if $a$ and $b$ can communicate and $b$ sends updates regarding level-$i$ neighbors to node $a$ according to Algorithm~\ref{alg:routes} (described later). Node $b$ is outbound for $a$; $a$ is an inbound neighbor for $b$.
...
\begin{samepage}
\begin{algo}
    A node $a$ builds its routing table $G$ (in the undirected graph representation of the network) by merging the graphs received from all of its outbound neighbors and adding all direct edges to these outbound neighbors. If node $a$ has an inbound neighbor $b$, it will send an appropriate subset $G_b$ of $G$ defined as following.

    Consider every graph chain $gc$ in $G$ with unique vertices starting with $a$. Now, define $gc'$ and $G'$ by adding node $b$ and the edge $b$--$a$ to $gc$ and $G$ respectively. All nodes and vertices of $gc$ will be included in $G_b$ if: 1) the color of the last node $col$ in $gc$ differs from the color of all other vertices in $gc$ and 2) there is no such node $c$ with color $col$ in $G'$ such that the shortest path from $a$ to $c$ and the shortest path from $c$ to $b$ are both shorter than the length of $gc'$.

    If node $b$ has just connected to $a$, $a$ will calculate and send out $G_b$. Later, for any change in $G$, $a$ will recalculate $G_b$ and send out the difference between the old and new $G_b$, such that $b$ can construct the new $G_b$ without incurring much traffic.

    \label{alg:routes}
\end{algo}
\end{samepage}

In Figure~\ref{fig:bmlrp-routes} node 0 connects to nodes 1 and 4. A directed edge from vertex $a$ to $b$ means $a$ is connected to $b$; a bidirectional link indicates connectivity in both directions. By Algorithm~\ref{alg:routes}, node's 1 graph $G'$ is depicted in Figure~\ref{fig:bmlrp-routes-nb1}. There are 6 different chains starting with node 1. Chains that will be included in $G_0$ are 1--5--4, 1--2--6. Chains 1--5, 1--2, 1--3 will not be included because the color of the last node is not unique. Finally, chain 1--2--3--7 will not be included because the shortest distance from 0 to 6 and from 6 to 7 is shorter than the chain itself. Figure~\ref{fig:bmlrp-routes-nb2} shows a similar graph of node 4. Here, the only 2 chains satisfying the requirement are 4--5 and 4--8. Node 0 combines the information received from the nodes 1,4 and forms a graph $G$ depicted in Figure~\ref{fig:bmlrp-routes-visible}.
...
To ensure connectivity between any two nodes in the network, it is sufficient to make every same-color node in level-$i$ graph $G$ an outbound neighbor on level-$(i+1)$. It can be foreseen, however, that this will lead to big routing tables on higher levels. Therefore, a more restrictive approach is needed.

\begin{algo}
    The following algorithm describes what nodes are necessary to connect to in order to satisfy the connectivity requirement. Node $a$ indicates the current node. $G$ is assumed to be routing table (graph) on level $i$. $N(x)$ indicates the set of all neighbors of $x$ in $G$. $connect(x)$ indicates $x$ will become an outbound level-$(i+1)$ neighbor of $a$.
    ...
    \begin{algorithmic}[1]
        ...
        \Function{ConnectToNecessaryNodes}{}
            ...
            \For {$b \in N(a)$}
                \If {color(b) == color(a)}
                    \State connect($b$)      \Comment {make $b$ an outbound neighbor on the next level}
                \Else
                    \State $used[b] \gets true$
